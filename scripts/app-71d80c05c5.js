...
(function() {
    ...
    angular.module("app.settings.login")
      .controller('LoginController', ["$scope", "$rootScope", "$state", "$http", "$cookieStore", "$mdDialog", function($scope, $rootScope, $state,$http,$cookieStore,$mdDialog) {
        var vm = this;
...
(function(){
  ...
  angular.module('app.appraisals.newappraisal').
      controller('NewAppraisalController',["AppConstants", "$http", "$scope", "$rootScope", function(AppConstants,$http,$scope,$rootScope){
          var vm = this;
          vm.responsibilities = [];
          vm.jobsummaries = [];
          vm.objective = {};
...
(function() {
  ...
  angular
    .module("app.appraisals.manageappraisal")
    .controller("ManageAppraisalController", ["$scope", "$mdDialog", "$document", "$http", "AppConstants", "UtilityService", function(
      $scope,
      $mdDialog,
      $document,
      $http,
...
(function() {
  'use strict';
    ...
    DashboardController.$inject = ["UtilityService"];
    DialogController.$inject = ["$scope", "$rootScope", "$mdDialog", "$mdMedia", "$mdEditDialog", "$q", "dialogData"];
  angular
      .module('app.dashboard')
...
$templateCache.put("app/main/dialogs/appraisalform.html","<md-dialog aria-label=\"Employee Appraisal Form\" ng-cloak draggable><div layout=\"column\" class=\"page\" layout-fill>\n<md-card class=\"default header\" layout=\"column\" layout-align=\"center left\"><h3><md-icon \nmd-font-icon=\"icon-content-paste\"></md-icon>Employee Appraisal Form</h3></md-card><fieldset id=\"viewfieldset\"><legend>\nEMPLOYEE PROFILE</legend><div class=\"personal-profile\" layout=\"row\"><div class=\"profile-details\" layout=\"column\" flex>\n<div class=\"header\"><h1>{{vm.selected.FullName||\'N/A\'}}</h1></div><div class=\"details\" layout=\"row\" layout-wrap><div \nflex=\"25\"><p>Line Of Business</p><span>{{vm.selected.lob||\'N/A\'}}</span></div><div flex=\"25\"><p>Recruitment Date</p>\n<span>{{(!vm.selected[0].Age)?\'N/A\':vm.selected[0].Age}}</span></div><div flex=\"25\"><p>Location/based at</p><span>\n{{(!vm.selected[0][\'Religion\'].Name)?\'N/A\':vm.selected[0][\'Religion\'].Name}}</span></div><div flex=\"25\"><p>\nYear or period covered</p><span>\n{{(!vm.selected[0][\'Patient\'].PhoneNumber)?\'N/A\':vm.selected[0][\'Patient\'].PhoneNumber}}</span></div><div flex=\"25\"><p>\nTime in present position</p><span>{{(!vm.selected[0][\'Gender\'].Name)?\'N/A\':vm.selected[0][\'Gender\'].Name}}</span></div>\n<div flex=\"25\"><p>Length of service</p><span>\n{{(!vm.selected[0][\'MaritalState\'].Name)?\'N/A\':vm.selected[0][\'MaritalState\'].Name}}</span></div><div flex=\"25\"><p>\nAppraisal date & time</p><span>{{(!vm.selected[0][\'Patient\'].PhoneNumber)?\'N/A\':vm.selected[0][\'Patient\'].PhoneNumber}}\n</span></div><div flex=\"25\"><p>Appraisal venue</p><span>\n{{(!vm.selected[0][\'Gender\'].Name)?\'N/A\':vm.selected[0][\'Gender\'].Name}}</span></div><div flex=\"25\"><p>Appraiser</p>\n<span>{{(!vm.selected[0][\'MaritalState\'].Name)?\'N/A\':vm.selected[0][\'MaritalState\'].Name}}</span></div></div></div>\n</div></fieldset><fieldset id=\"viewfieldset\"><legend>GOALS & OBJECTIVES</legend><div class=\"personal-profile\" \nlayout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><div class=\"details\" layout=\"row\" layout-wrap><div \nflex=\"25\"><span>{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div></div></div>\n</div></fieldset><fieldset id=\"viewfieldset\"><legend>EMPLOYEE’S SELF- ASSESSMENT</legend><div class=\"personal-profile\" \nlayout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><div class=\"visits\" flex><h2 class=\"heading\"><md-icon \nmd-svg-src=\"./assets/icons/svg/added/Admit.svg\"></md-icon>Job Summary</h2><div class=\"single-visit\"><md-grid-list \nmd-cols=\"2\" md-gutter=\"1em\" md-row-height=\"15px\" style=\"width: 300px\"><md-grid-tile>\n{{\'MY_PATIENT.WARD_NUMBER\'|translate}}</md-grid-tile><md-grid-tile>{{vm.selected[0][\'Department\'].Name}}</md-grid-tile>\n<md-grid-tile>{{\'MY_PATIENT.BED_NUMBER\'|translate}}</md-grid-tile><md-grid-tile>{{vm.selected[0][\'Bed\'].Number}}\n</md-grid-tile></md-grid-list></div></div><div class=\"visits\" flex><h2 class=\"heading\"><md-icon \nmd-svg-src=\"./assets/icons/svg/added/Admit.svg\"></md-icon>Adjusted Responsibilities/Additional Comments</h2><div \nclass=\"single-visit\"><md-grid-list md-cols=\"2\" md-gutter=\"1em\" md-row-height=\"15px\" style=\"width: 300px\"><md-grid-tile>\n{{\'MY_PATIENT.WARD_NUMBER\'|translate}}</md-grid-tile><md-grid-tile>{{vm.selected[0][\'Department\'].Name}}</md-grid-tile>\n<md-grid-tile>{{\'MY_PATIENT.BED_NUMBER\'|translate}}</md-grid-tile><md-grid-tile>{{vm.selected[0][\'Bed\'].Number}}\n</md-grid-tile></md-grid-list></div></div></div></div></fieldset><fieldset id=\"viewfieldset\"><legend>\nPERFORMANCE APPRAISAL</legend><div class=\"personal-profile\" layout=\"row\"><div class=\"profile-details\" layout=\"column\" \nflex><div class=\"details\" layout=\"column\" layout-wrap><div flex><p style=\"font-weight: bold; font-size:15px\">\nAccomplishment of key responsibilities/deliverables:</p><span style=\"margin:20px\">\n{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">Achievement of the goals established during the past year:</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0].Age)?\'N/A\':vm.selected[0].Age}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">\nIdentify areas of exceptional performance on your part that should be particularly noted. Provide specific examples.\n</p><span style=\"margin:20px\">{{(!vm.selected[0][\'Religion\'].Name)?\'N/A\':vm.selected[0][\'Religion\'].Name}}</span></div>\n<div flex><p style=\"font-weight: bold; font-size:15px\">\nIn your current position, can you share an example where you have demonstrated your leadership? Provide specific examples.\n</p><span style=\"margin:20px\">{{(!vm.selected[0][\'Patient\'].PhoneNumber)?\'N/A\':vm.selected[0][\'Patient\'].PhoneNumber}}\n</span></div><div flex><p style=\"font-weight: bold; font-size:15px\">\nIn your current position, can you share an example where you have taken ownership? Provide specific examples.</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0][\'Gender\'].Name)?\'N/A\':vm.selected[0][\'Gender\'].Name}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">\nIdentify areas of your performance needing more attention or improvement. Provide specific examples.</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0][\'MaritalState\'].Name)?\'N/A\':vm.selected[0][\'MaritalState\'].Name}}</span></div>\n<div flex><p style=\"font-weight: bold; font-size:15px\">\nApart from your current role, is there any other role in the organization you will like to function in? Please specify the role, state the reasons for your choice and articulate how you intend to contribute to the goal of the organization by functioning in this role\n</p><span style=\"margin:20px\">{{(!vm.selected[0][\'MaritalState\'].Name)?\'N/A\':vm.selected[0][\'MaritalState\'].Name}}\n</span></div></div></div></div></fieldset><fieldset id=\"viewfieldset\"><legend>FOUNDING PHILOSOPHY</legend><div \nclass=\"personal-profile\" layout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><md-table-container><table \nmd-table md-row-select=\"options.rowSelection\" multiple=\"{{options.multiSelect}}\" ng-model=\"selected\" \nmd-progress=\"promise\"><thead ng-if=\"!options.decapitate\" md-head md-order=\"query.order\" md-on-reorder=\"logOrder\"><tr \nmd-row><th md-column md-order-by=\"name\"><span>Creed </span></th><th md-column md-order-by=\"type\"><span>Definitions\n</span></th><th md-column md-order-by=\"type\"><span>Ratings</span></th></tr></thead><tbody md-body><tr md-row \nmd-select=\"user\" md-on-select=\"logItem\" md-auto-select=\"options.autoSelect\" \nng-repeat=\"philosophy in vm.foundingPhilosophy\"><td md-cell>{{philosophy.name}}</td><td md-cell>\n{{philosophy.definition}}</td><td md-cell>\n{{philosophy.rating==4?\'Exceptional\':philosophy.rating==3?\'Good\':philosophy.rating==2?\'Below Average\':\'Poor\'}}</td>\n</tr></tbody></table></md-table-container><p></p><div flex><p style=\"font-weight: bold; font-size:15px\">\nKey achievement / client feedback on my performance in relation to the founding philosophy</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div><p></p>\n</div></div></fieldset><fieldset id=\"viewfieldset\"><legend>OVERALL ASSESSMENT</legend><div class=\"personal-profile\" \nlayout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><div class=\"details\" layout=\"row\" layout-wrap><div \nflex=\"25\"><span>{{ (!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number }}</span></div></div>\n</div></div></fieldset><fieldset id=\"viewfieldset\"><legend>DEVELOPMENT PLAN</legend><div class=\"personal-profile\" \nlayout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><div class=\"details\" layout=\"row\" layout-wrap><fieldset>\n<legend>Performance Enhancement (Current Position)</legend><div flex><p style=\"font-weight: bold; font-size:15px\">\nAccomplishment of key responsibilities/deliverables:</p><span style=\"margin:20px\">\n{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">Achievement of the goals established during the past year:</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0].Age)?\'N/A\':vm.selected[0].Age}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">\nIdentify areas of exceptional performance on your part that should be particularly noted. Provide specific examples.\n</p><span style=\"margin:20px\">{{(!vm.selected[0][\'Religion\'].Name)?\'N/A\':vm.selected[0][\'Religion\'].Name}}</span></div>\n</fieldset><fieldset><legend>Career Development (Future Positions)</legend><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">Accomplishment of key responsibilities/deliverables:</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div><div \nflex><p style=\"font-weight: bold; font-size:15px\">Achievement of the goals established during the past year:</p><span \nstyle=\"margin:20px\">{{(!vm.selected[0].Age)?\'N/A\':vm.selected[0].Age}}</span></div><div flex><p \nstyle=\"font-weight: bold; font-size:15px\">\nIdentify areas of exceptional performance on your part that should be particularly noted. Provide specific examples.\n</p><span style=\"margin:20px\">{{(!vm.selected[0][\'Religion\'].Name)?\'N/A\':vm.selected[0][\'Religion\'].Name}}</span></div>\n</fieldset></div></div></div></fieldset><fieldset id=\"viewfieldset\"><legend>APPROVE/DECLINE</legend><div \nclass=\"personal-profile\" layout=\"row\"><div class=\"profile-details\" layout=\"column\" flex><div class=\"details\" \nlayout=\"row\" layout-wrap><div layout=\"row\" flex><div flex=\"10\"><label class=\"tiredlabel\"><span class=\"\">*</span>\n Comment:</label></div><md-input-container class=\"tiredinput\" flex=\"80\"><label></label><textarea md-no-asterisk \ntype=\"text\" ng-model=\"vm.profiledata.FullName\"></textarea></md-input-container></div></div></div></div></fieldset><div \nlayout=\"row\" layout-align=\"center center\"><md-button class=\"md-raised md-accent\" ng-click=\"cancel()\" \nstyle=\"margin-right:20px\">Approve</md-button><md-button class=\"md-raised md-accent\" ng-click=\"cancel()\" \nstyle=\"margin-right:20px\">Decline</md-button><md-button class=\"md-raised md-accent\" ng-click=\"cancel()\" \nstyle=\"margin-right:20px\">Close</md-button></div></div></md-dialog>");
...
$templateCache.put("app/main/apps/dashboard/dashboard.html","<div layout=\"row\" id=\"patient-billing\" ng-cloak=\"\" layout-fill><div layout=\"column\" class=\"page\" flex><md-card \nclass=\"default header\" layout=\"row\" layout-align=\"center\"><h3></h3></md-card><md-content class=\"body\" layout=\"column\" \nflex layout-margin ms-scroll><fieldset id=\"viewfieldset\"><div class=\"personal-profile\" layout=\"row\"><div \nclass=\"profile-details\" layout=\"column\" flex><div class=\"header\"><h1>\n{{(vm.selected[0][\'Patient\'].Surname&&vm.selected[0][\'Patient\'].OtherNames)? vm.selected[0][\'Patient\'].Surname +\' \'+ vm.selected[0][\'Patient\'].OtherNames:vm.selected[0].Name||\'N/A\'}}\n<md-button class=\"md-icon-button\" ng-click=\"vm.RespondToQuickAction(\'edit\')\" ng-show=\"vm.showEditButton\" \nng-disabled=\"vm.selected.length==0\"><md-icon md-font-icon=\"icon-pencil\"></md-icon></md-button></h1></div><div \nclass=\"details\" layout=\"row\" layout-wrap><div flex=\"25\"><p>Line Of Business</p><span>\n{{(!vm.selected[0][\'Patient\'].Number)?\'N/A\':vm.selected[0][\'Patient\'].Number}}</span></div><div flex=\"25\"><p>\nRecruitment Date</p><span>{{(!vm.selected[0].Age)?\'N/A\':vm.selected[0].Age}}</span></div><div flex=\"25\"><p>\nLocation/based at</p><span>{{(!vm.selected[0][\'Religion\'].Name)?\'N/A\':vm.selected[0][\'Religion\'].Name}}</span></div>\n<div flex=\"25\"><p>Phone Number</p><span>\n{{(!vm.selected[0][\'Patient\'].PhoneNumber)?\'N/A\':vm.selected[0][\'Patient\'].PhoneNumber}}</span></div><div flex=\"25\"><p>\nEmail</p><span>{{(!vm.selected[0][\'Gender\'].Name)?\'N/A\':vm.selected[0][\'Gender\'].Name}}</span></div><div flex=\"25\"><p>\nDepartment/Unit</p><span>{{(!vm.selected[0][\'MaritalState\'].Name)?\'N/A\':vm.selected[0][\'MaritalState\'].Name}}</span>\n</div></div></div></div></fieldset></md-content></div></div>");
...
//# sourceMappingURL=../maps/scripts/app-71d80c05c5.js.map
